...
6. Website = Mode B (light/editorial). Blog featured images = Mode A (dark).
7. Photography featured LARGE. Never reduce to small avatar.
8. Sharp-cornered buttons only. No pill buttons on website.
9. Verify URLs before including in any content.
10. Fetch Notion pages before doing any brand/content/visual work.
11. When updating any Notion strategy/SSOT page (Brand Voice, Visual System, Creative Direction,
    Strategy, Marketing Strategy, Content Engine, Hook Strategy, etc.): always show exactly what
    was added, changed, or removed, and provide the clickable Notion URL so Florian can verify
    formatting. Never update these pages silently.

## Social Media Visual Direction (March 2026)
Target: Magazine covers, not LinkedIn templates.
Reference: Johnson Gill / Lark Creatives (see Competitive Landscape in Notion).
Formula: Oversized type interacting with B&W photography of Florian + purple accent.
One idea per post, massively scaled. No bullet lists, no icons.
Photography-forward for social. Prism imagery reserved for blog thumbnails and conceptual contexts only.

## Folder Structure
```
website/          — Active homepage builds
brand-assets/     — Photography, imagery, social examples (brand reference, git-tracked)
content/          — Production content media (git-tracked, GitHub-hosted for Buffer)
  social/         — Social media assets (per-post folders: YYYY-MM-DD-slug/)
  video/          — Video assets
  _staging/       — Landing zone for Gemini/Figma exports before publishing
deliverables/     — Moodboards, proposals, decks
archive/          — Old versions, experiments
```

Note: Text content (captions, blog posts) lives in Notion only. Local content/ is media files only.
GitHub hosts web-optimized files for Buffer API (requires hosted URLs for images).
